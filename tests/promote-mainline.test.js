...
describe('Promote mainline', () => {
  it('Promote mainline: Only mainline moves', () => {
    ...
    // Assert
    expect(newMoments).to.be.an('array').that.has.lengthOf(5);
  ...
  it('Promote mainline: One sideline', () => {
    ...
    // Assert
    expect(newMoments).to.be.an('array').that.has.lengthOf(7);
  ...
  it('Promote mainline: Two sidelines', () => {
    ...
    expect(newMoments).to.be.an('array').that.has.lengthOf(9);
    expect(newMoments[2]?.move).to.equal('c5');
    expect(newMoments[2]?.depth).to.equal(1);
    expect(newMoments[4]?.move).to.equal('e5');
    expect(newMoments[4]?.depth).to.equal(2);
  ...
  it('Promote mainline: Sub-sideline promotion', () => {
    // Arrange
    const pgn = '1. e4 e5 2. Nf3 (2. Bc4 Nf6 (2... Bc5)) 2... Nc6 *';
    const moments = flat(pgn);
    const current = findMoment(moments, 'Bc5');

    // Act
    const newMoments = promoteMainline(moments, current);

    // Assert
    // Expected result: 1. e4 e5 2. Nf3 (2. Bc4 Bc5 (2... Nf6)) 2... Nc6 *
    expect(newMoments).to.be.an('array').that.has.lengthOf(11);
    expect(newMoments[6]?.move).to.equal('Bc5');
    expect(newMoments[6]?.depth).to.equal(2);
    expect(newMoments[8]?.move).to.equal('Nf6');
    expect(newMoments[8]?.depth).to.equal(3);
    // Verify mainline moves remain unchanged
    expect(newMoments[3]?.move).to.equal('Nf3');
    expect(newMoments[3]?.depth).to.equal(1);
    expect(newMoments[10]?.move).to.equal('Nc6');
...
describe('Promote mainline: Fischer vs. Andersson', () => {
  ...
  it('Promote mainline: Promote 13. d4 sideline', () => {
    const moments = flat(pgn);
    const current = findMoment(moments, 'd4');

    // Act
    const newMoments = promoteMainline(moments, current);
    const nextCurrent = findMoment(newMoments, 'exd4');
  ...
  it('Promote mainline: Promote 16. Bb2 sideline', () => {
    const moments = flat(pgn);
    const current = findMoment(moments, 'Bb2');

    // Act
    const newMoments = promoteMainline(moments, current);

    // Assert
    expect(newMoments[9]?.move).to.equal('Bb2');
    expect(newMoments[9]?.depth).to.equal(2);
    expect(newMoments[11]?.move).to.equal('Bxc5');
    expect(newMoments[11]?.depth).to.equal(3);
  });
});
